...
std::vector<StarCount> countStars(odb::database& db, float latMin, float latMax, float longMin,
	...
	transaction t(db.begin());
	// Your implementation goes here:
	sc_result_t results = db.query<StarCount>(std::string("SELECT stars, COUNT(*) FROM (SELECT review.stars FROM review,business WHERE review.business_id = business.id AND ") +
